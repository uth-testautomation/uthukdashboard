...
@then(u'the user analyzes the call forward unconditional traces')
def analysing_traces_for_cfu(context):
    try:
        ...
        # context.CFU_to_third_party_Sub_A = flags["Flag_200_OK_Bye"]
        ...
        # context.CFU_to_third_party_Sub_B = flags["Flag_CF_Unconditional"]
...
@then(u'then the CFU to third party traces are validated')
def validation_of_log_for_CFU(context):
    try:
        if context.FlagA_181_call_forwarded and context.FlagA_183_Session_in_Progress_2 and context.FlagB_CF_Unconditional:
            with allure.step('CFU to third party: PASS'):
                pass
        else:
            with allure.step('CFU to third party: FAIL'):
                assert False, "Test Failed"
    except Exception as e:
        print(f"Exception: {e}")