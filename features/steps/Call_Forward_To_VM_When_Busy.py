...
@then(u'the user analyzes the call forward to Voice Mail when busy traces')
def analysing_traces_for_cfb_to_VM(context):
    try:
        ...
        # context.CFB_to_Voicemail_Sub_A = flags["Flag_200_OK_Bye"]
        ...
        flag_messages = {
            ...
            "SIPMSG[0]: [<--] SIP/2.0 200 OK [CSeq: 1 INVITE]" : "FlagB_200_ok_Invite",
        ...
        # context.CFB_to_Voicemail_Sub_B = flags["Flag_202_Accepted"]
...
@then(u'then the CFB to Voice Mail is validated')
def validation_of_log_for_CFB_to_VM(context):
    try:
        if context.FlagA_183_Session_in_Progress and context.FlagA_181_call_forwarded and context.FlagA_200_OK_Bye and context.FlagB_486_busy_here and context.FlagB_202_Accepted:
            with allure.step('CFB Voice Mail Deposit:PASS'):
                pass
        else:
            with allure.step('CFB Voice Mail Deposit:FAIL'):
                assert False, "Test Failed"
    except Exception as e:
        print(f"Exception: {e}")