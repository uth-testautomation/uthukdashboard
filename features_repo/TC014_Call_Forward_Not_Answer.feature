...
Scenario Outline:  Call forwarding to third party when user doesn't answer
    ...
    Then user terminates the call from "subscriber_A" after "<Call-duration>" seconds
    ...
    Then the user analyzes the call forward not answered traces
    And then the CFNA to third party is validated
